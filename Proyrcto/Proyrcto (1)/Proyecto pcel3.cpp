...
void inciar_facil (char[IF][JF],int*,int*,struct puntaje*,struct pasos*);
void inciar_medio (char[IM][JM],int*,int*,struct puntaje*,struct pasos*);
void imprimir_tablero (char [IF][JF],struct jugador,struct puntaje ,int,int,struct pasos);
void imprimir_tablero_medio (char [IM][JM],struct jugador,struct puntaje,int,int,struct pasos);
void mover_arriba (char [IF][JF],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_arriba_medio (char [IM][JM],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_abajo (char [IF][JF],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_abajo_medio (char [IM][JM],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_derecha (char [IF][JF],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_derecha_medio (char [IM][JM],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_izquierda (char [IF][JF],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void mover_izquierda_medio (char [IM][JM],struct jugador,struct puntaje*,int*,int*,struct pasos*);
void despedida (void);
main ()
            ...
            switch(opc)
            {
                case '1':
                    ...
                    do
                    {
                        ...
                        if(niv=='1')
                        {
                            inciar_facil (tab1,&i,&j,&puntos,&pas);
                            imprimir_tablero (tab1,nom,puntos,i,j,pas);
                                do
                                {
                                    ...
                                    {
                                        mover_arriba(tab1,nom,&puntos,&i,&j,&pas);
                                    }
                                    else    if(mov=='d' || mov=='D')    /** DERECHA **/
                                    {
                                        mover_derecha (tab1,nom,&puntos,&i,&j,&pas);
                                    }
                                    else    if(mov=='s' || mov=='S')    /** ABAJO **/
                                    {
                                        mover_abajo (tab1,nom,&puntos,&i,&j,&pas);
                                    }
                                    else    if(mov=='a' || mov=='A')    /** IZQUIERDA **/
                                    {
                                        mover_izquierda (tab1,nom,&puntos,&i,&j,&pas);
                                    }
                                }while((tab1[8][1]==70)&&(puntos.total<100)&&(mov!=42));
                                nom.puntaje=puntos.total;
                                if(tab1[8][1]!=70)
                                {
                                    system("cls");
                                    printf("\n\n\n\n\n\n\t\tFELICITACIONES..... JUEGO COMPLETO\n\n\n\n\n");
                                    printf("\n\n\n\t\tNOMBRE: %s\n\n\n\t\tPUNTOS TOTALES: %d",nom.nombre,nom.puntaje);
                                    getch();
                                }
                                if(puntos.total>=100)
                                {
                        ...
                        {
                            inciar_medio (tab2,&i,&j,&puntos,&pas);
                            imprimir_tablero_medio (tab2,nom,puntos,i,j,pas);
                            do
                            {
                                mov=getch();    /**MOVER MACEL**/
                                if(mov=='w' || mov=='W')    /** ARIBA **/
                                {
                                    mover_arriba_medio(tab2,nom,&puntos,&i,&j,&pas);
                                }
                                else    if(mov=='d' || mov=='D')    /** DERECHA **/
                                {
                                    mover_derecha_medio (tab2,nom,&puntos,&i,&j,&pas);
                                }
                                else    if(mov=='s' || mov=='S')    /** ABAJO **/
                                {
                                    mover_abajo_medio (tab2,nom,&puntos,&i,&j,&pas);
                                }
                                else    if(mov=='a' || mov=='A')    /** IZQUIERDA **/
                                {
                                    mover_izquierda_medio (tab2,nom,&puntos,&i,&j,&pas);
                                }
                            }while((tab2[36][1]==70)&&(puntos.total<1000)&&(mov!=42));
                            nom.puntaje=puntos.total;
                            if(tab2[36][1]!=70)
                            {
                                system("cls");
...
void inciar_facil (char tab1[IF][JF],int*i,int*j,struct puntaje*pun,struct pasos*pas)
{
    int filas, columnas;
...
void inciar_medio (char tab2[IM][JM],int*i,int*j,struct puntaje*pun,struct pasos*pas)
{
    int filas, columnas;
    ...
    pas->muro=0;
    pas->limite=0;
    *i=4,*j=13,pun->total=0,tab2[1][7]=M,tab2[13][5]=M,tab2[3][13]=M,tab2[13][13]=M,tab2[14][7]=M,tab2[35][7]=M,tab2[4][11]=M,tab2[3][2]=M,tab2[4][4]=M,tab2[11][3]=M,tab2[3][8]=M,tab2[3][9]=M,tab2[34][4]=M,tab2[29][11]=M,tab2[25][7]=M,tab2[23][12]=M,tab2[33][2]=M;
...
void imprimir_tablero (char tablero[IF][JF],struct jugador nom,struct puntaje punt,int i,int j,struct pasos pas)   /**IMPRIMIR**/
{
    int fil,col;
    system("cls");
    printf("\n\n\n\n\n\n");
    ...
    punt.res=100-punt.total;
    printf("\n\n\t\tNOMBRE: %s",nom.nombre);
    printf("\n\n\t\tPUNTOS: %d",punt);
    printf("\n\n\t\tPUNTOS RESTANTES; %d",punt.res);
    printf("\n\n\t\tPASOS HACIA ARRIBA; %d",pas.arriba);
    printf("\n\n\t\tPASOS HACIA ABAJO; %d",pas.abajo);
    printf("\n\n\t\tPASOS HACIA DERECHA; %d",pas.derecha);
    printf("\n\n\t\tPASOS HACIA IZQUIERDA; %d",pas.izquierda);
    printf("\n\n\t\tPULSE * PARA SALIR");
}
void imprimir_tablero_medio (char tablero[IM][JM],struct jugador nom,struct puntaje punt,int i,int j,struct pasos pas)   /**IMPRIMIR**/
{
    int fil,col;
    system("cls");
    printf("\n\n\n\n\n\n");
...
void mover_arriba (char tab[IF][JF],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
    if(tab[*i-1][*j]==64)
    {
        punt->total=punt->total+5;
        tab[*i][*j]=W;   /** (^) **/
    }
    else if(tab[*i-1][*j]==42)
    {
        punt->total=punt->total+3;
        tab[*i][*j]=W;   /** (^) **/
    }
    else if(tab[*i-1][*j]==32)
    {
        punt->total=punt->total+1;
        tab[*i][*j]=E;   /** (ESPACIO) **/
        *i=*i-1;
        tab[*i][*j]=W;   /** (^) **/
    }
    pas->arriba=pas->arriba+1;
    imprimir_tablero (tab,nom,*punt,*i,*j,*pas);
}
void mover_arriba_medio (char tab[IM][JM],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
    if(tab[*i-1][*j]==64)
    ...
    pas->arriba=pas->arriba+1;
    imprimir_tablero_medio (tab,nom,*punt,*i,*j,*pas);
}
void mover_abajo (char tab[IF][JF],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
    if(tab[*i+1][*j]==64)
    ...
    pas->abajo=pas->abajo+1;
    imprimir_tablero (tab,nom,*punt,*i,*j,*pas);
}
void mover_abajo_medio (char tab[IM][JM],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
    if(tab[*i+1][*j]==64)
...
void mover_derecha (char tab[IF][JF],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
    if(tab[*i][*j+1]==64)
    ...
    pas->derecha=pas->derecha+1;
    imprimir_tablero (tab,nom,*punt,*i,*j,*pas);
}
void mover_derecha_medio (char tab[IM][JM],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
    if(tab[*i][*j+1]==64)
...
void mover_izquierda (char tab[IF][JF],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
        if(tab[*i][*j-1]==64)
    ...
    pas->izquierda=pas->izquierda+1;
    imprimir_tablero (tab,nom,*punt,*i,*j,*pas);
}
void mover_izquierda_medio (char tab[IM][JM],struct jugador nom,struct puntaje*punt,int*i,int*j,struct pasos*pas)
{
    int temp;
    system("cls");
        if(tab[*i][*j-1]==64)
